...
CREATE POLICY "Users can view employees for their subaccounts" ON "SubAccountEmployee"
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM "SubAccount" 
...
CREATE POLICY "Users can add employees to their subaccounts" ON "SubAccountEmployee"
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM "SubAccount" 
            WHERE "SubAccount"."id" = "SubAccountEmployee"."subAccountId" 
...
CREATE POLICY "Users can update employees in their subaccounts" ON "SubAccountEmployee"
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM "SubAccount" 
            WHERE "SubAccount"."id" = "SubAccountEmployee"."subAccountId" 
...
CREATE POLICY "Users can delete employees from their subaccounts" ON "SubAccountEmployee"
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM "SubAccount" 
            WHERE "SubAccount"."id" = "SubAccountEmployee"."subAccountId" 
...
CREATE POLICY "Users can upload files to subaccount folders" ON storage.objects
    FOR INSERT WITH CHECK (
        bucket_id = 'files' AND
        auth.uid() IS NOT NULL AND
        (storage.foldername(name))[1] = 'subaccount-files'
    );

CREATE POLICY "Users can view files in subaccount folders" ON storage.objects
    FOR SELECT USING (
        bucket_id = 'files' AND
        auth.uid() IS NOT NULL AND
        (storage.foldername(name))[1] = 'subaccount-files'
...
COMMENT ON TABLE "SubAccountFile" IS 'Stores file attachments for subaccounts';
COMMENT ON TABLE "SubAccountEmployee" IS 'Stores employee information for subaccounts';
...
COMMENT ON COLUMN "SubAccountEmployee"."role" IS 'Employee role/position';
COMMENT ON COLUMN "SubAccountEmployee"."isActive" IS 'Whether the employee is currently active';
COMMENT ON COLUMN "SubAccountReport"."status" IS 'Current status of the report';
COMMENT ON COLUMN "SubAccountReport"."assignedTo" IS 'Employee assigned to work on this report';
