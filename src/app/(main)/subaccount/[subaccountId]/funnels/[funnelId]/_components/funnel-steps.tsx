    ...
    if (pages && pages.length >= 0) {
      ...
      if (sortedPages.length > 0) {
        setClickedPage(prevClicked => {
          if (!prevClicked || !sortedPages.find(p => p.id === prevClicked.id)) {
            console.log('🔄 Setting clickedPage to first page:', sortedPages[0].name)
            return sortedPages[0]
          }
          console.log('✅ Keeping clickedPage:', prevClicked.name)
          return prevClicked
        })
      } else {
