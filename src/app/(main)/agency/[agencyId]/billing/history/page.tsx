...
const BillingHistoryPage = async ({ params }: Props) => {
    ...
    // 4. Fetch ALL charges from Paystack (defaults to 50 latest)
    ...
    const allCharges = filteredTransactions.map((transaction: any) => ({
    ...
    return (
        <div className="flex flex-col gap-10 pb-20 p-4 md:p-8 relative">
            <div className="space-y-4">
                <h1 className="text-4xl font-bold tracking-tight">Billing History</h1>
                <p className="text-muted-foreground text-lg max-w-2xl">
                    View and manage your transaction history. Track your subscriptions, one-time payments, and billing events in one place.
                </p>
                <div className="h-1 w-20 bg-emerald-500 rounded-full" />
            </div>

            <BillingHistoryTable data={allCharges} />
