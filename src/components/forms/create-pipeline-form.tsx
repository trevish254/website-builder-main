  ...
  const onSubmit = async (values: z.infer<typeof CreatePipelineFormSchema>) => {
    if (!subAccountId) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'Subaccount ID is required',
      })
      return
    }
    
    try {
      console.log('🔧 Creating pipeline with values:', values)
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `${defaultData ? 'Updated' : 'Created'} a pipeline | ${response?.name}`,
        subaccountId: subAccountId,
      })
      ...
      if (response?.id) {
        router.push(`/subaccount/${subAccountId}/pipelines/${response.id}`)
      } else {
        router.refresh()
      }
    } catch (error) {
