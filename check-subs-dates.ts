...
async function checkAllUserSubsDetails() {
    ...
    if (res.status && Array.isArray(res.data)) {
        ...
        userSubs.forEach((s: any, i: number) => {
            console.log(`SUB #${i + 1}: Status: ${s.status}, Plan: ${s.plan.name} (${s.plan.plan_code}), CreatedAt: ${s.createdAt}`)
        })
    } else {
